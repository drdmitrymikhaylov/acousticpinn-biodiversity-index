...
number that means *how much life is here* takes two things. One is a classifier that
knows which sounds came from animals. The other is an index that behaves sensibly when
the community changes. This repository builds both on open data and measures where
each one breaks.
...
field recordings to produce a detection time series. The **Acoustic Biodiversity
...
This repository is the open half. It writes the index out as runnable code and asks
the question the paper had no room for: **does it move the right way?**
...
site with twenty species heard once each in a single hour is not biodiverse, yet an
...
thing that makes an index usable by someone else. It measures how the index behaves.

## Degradation scenarios

Four degradations, 3,000 synthetic communities, composition known by construction.
In every panel the community gets **worse from left to right**. A rising line is
therefore a measure rewarding a worse site.
...
something worse, and the section after next is about that.
...
is steady and fairly loud. That is the profile that flatters *all three* components
...
from the same 3,000-community run, it does not. Values are means over 40 communities
per level. The last column counts the communities in which the index is *higher* than
it was for that same community intact.
...
0.742 at 21 species. It does not fall back below its intact value until **eight
...
species present. A Poisson count at rate μ has a coefficient of variation near 1/√μ,
so the rarest species are the noisiest by construction. In the intact community the
three commonest species have CV 0.16 to 0.22 and the three rarest have 0.96 to 1.41.
Remove the rare ones and the mean CV of what is left drops from 0.54 to 0.36. **The
stability term penalises a species for being rare, not for being bursty.** A
...
al. 2011). **Biophony** is the animals. **Geophony** is wind, rain, water, thunder.
**Anthropophony** is everything humans make. 29 ESC-50 categories fall cleanly into
the three, 1,160 clips in all. Categories that are indoor or ambiguous in a forest are
...
three-way head there is a saw detector, 80 clips against 1,080. It is scored by
ROC-AUC and balanced accuracy because at 7% positives accuracy means nothing.
...
0.849 is the ceiling for this model on this data.
...
- **ESC-50 is not a forest.** It is curated Freesound audio, one dominant source per
  ...
  engine simultaneously. The three-way accuracy here is an upper bound on what the
  same model would do on overlapping sources.
...
The audio half was not written from scratch. `melspec.py` and `model.py` are the same
files used in the cough repository linked below, so the mel front end and the CNN
were already known to run before any forest data was touched. What took time was the
data. Xeno-canto's API v3 needs an account key and was not used; the ICBHI and
Rainforest Connection sets sit behind Kaggle logins. ESC-50 was the only fully open
